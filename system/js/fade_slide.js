...
slideengine:function(divid, action){
	...
	if (this.divholders[divid] && $divref.length==1){ 
		...
		$divref.animate(animateSetting, $divref.attr('speed')? parseInt($divref.attr('speed')) : 500, function(){
			...
			if (fadeslide.ontoggle){
				try{
					fadeslide.ontoggle(jQuery, $divref.get(0), $divref.css('display'))
				}
				catch(e){
...
init:function(){
	...
	jQuery(document).ready(function($){
		fadeslide.ontoggle=fadeslide.ontoggle || null
		var urlparamopenids=fadeslide.urlparamselect()
		...
		if (fadeslide.ontoggle){
			jQuery.each(ac.divholders, function(){ 
				fadeslide.ontoggle(jQuery, this.$divref.get(0), this.$divref.css('display'))
			})
		}
		...
		$allcontrols.each(function(){
			...
			if (this.getElementsByTagName('img').length==1 && ac.divholders[this._divids]){ 
				fadeslide.preloadimage(this.getAttribute('data-openimage'), this.getAttribute('data-closedimage'))
			...
			$(this).click(function(){ 
				...
				if (divids.length>0){
					fadeslide[/expand/i.test(relattr)? 'show' : /collapse/i.test(relattr)? 'hide' : 'toggle'](divids)
					return false
				}
			}) 
		})
...
//Setting Boxes
fadeslide.ontoggle=function($, divobj, state){ 
}
fadeslide.init()

//Defining Boxes Name
fadeslide.addDiv('top-box', 'fade=1')
fadeslide.addDiv('searchbox', 'fade=1')
fadeslide.addDiv('user-summery', 'fade=1,group=userdetails')
fadeslide.addDiv('user-invoice', 'fade=1,group=userdetails')
fadeslide.addDiv('user-service', 'fade=1,group=userdetails')
fadeslide.addDiv('user-ticket', 'fade=1,group=userdetails')
fadeslide.addDiv('user-prepaid', 'fade=1,group=userdetails')
fadeslide.addDiv('user-affreq', 'fade=1,group=userdetails')
fadeslide.addDiv('affil-payment', 'fade=1')
fadeslide.addDiv('forgot-pass', 'fade=1')
fadeslide.addDiv('default-lang', 'fade=1')
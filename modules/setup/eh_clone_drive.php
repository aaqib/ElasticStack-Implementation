...
if(isset($_POST['act']) && $_POST['act'] != '')
{
	...
	//validation
	$Form->ValidField($image,'empty','Select Clone Drive.');$Form->iferror();
	if($drive_type == '0')
	{
	...
	if($entry==1) //if not validation error
	{
		...
		if($image != '') //maping drive to drive
		{
			eh_map_image($drive, $image, $encryption);
			$arr = array("type"=>"Elastichosts_Clone_Drive", "activity"=>"Elastichosts Clone Drive <b>ID-".$drive."</b>", 'c_type'=>0, "id"=>$id, "client"=>$_SESSION['usrId']); addActivity($arr);
...
<h1>Clone Drive</h1></div><div class="midcont-contbox">
...
<tr>
<td class="inr-det-row2" style='width:30%'>Clone Drive:</td>
<?php 
	$input_key	= 'image';
...
<td colspan="2" class="inr-det-row6">
	<input class="inr-edit-btn3" type="submit" name="Submit" value="Start Cloning" />
</td>
</tr>
</table>

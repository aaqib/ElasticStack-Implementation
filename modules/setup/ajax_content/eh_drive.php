...
{
	$row[0]	= $key;
	//$result	= eh_get_drive_info($drive);
	$result	= $value;
	//var_dump($result);

	$imaging	= ($result['imaging'] != '') ? ' (<span id="'.$row[0].'" class="js-reload">'.$result['imaging'].'</span>)' : '';
...
?>
	...
	<td class="inr-tabrow3"><?=$result['drive'];?></td>
	<td class="inr-tabrow3"><?=$result['size'];?> (<?php  echo number_format(($result['size'] / 1024 / 1024 / 1024), 1); ?> GB)</td>
	<td class="inr-tabrow3"><?=$result['tier'];?></td>
	<td class="inr-tabrow3"><?=$result['status'].$imaging;?></td>
	<td class="inr-tabrow3"><?=$result['encryption:cipher'];?></td>
	<td class="inr-tabrow4"><a class="inr-tablnk1" href="eh_drive_add_edit.php?id=<?=$row[0]?>">Edit Details</a></td>
	</tr>
<?php 
}
?>

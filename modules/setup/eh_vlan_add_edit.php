...
if(isset($_POST['act']) && $_POST['act'] != '')
{
	...
	//validation
	$Form->ValidField($name,'empty','Enter VLAN Name.');$Form->iferror();
	...
	if($entry==1) //if not validation error
	{
		...
		if($act == "add")
		{
			$info	= eh_create_resource('vlan', $params);
			if(is_array($info))
			{
				$arr = array("type"=>"Elastichosts_VLAN", "activity"=>"Add Elastichosts VLAN <b>Name-".$info['name']."</b>", 'c_type'=>0, "id"=>$id, "client"=>$_SESSION['usrId']); addActivity($arr);
		...
		{
			$info	= eh_update_resource('vlan', $vlan, $params);
			if(is_array($info))
			{
				$arr = array("type"=>"Elastichosts_VLAN", "activity"=>"Update Elastichosts VLAN <b>Name-".$info['name']."</b>", 'c_type'=>0, "id"=>$id, "client"=>$_SESSION['usrId']); addActivity($arr);
				header("Location:".$return_page);
				exit;
			}
			else
...
if(isset($id) && $id != '')
{
	$result	= eh_get_resource_info('vlan', $id);
}	
?>
<!DOCTYPE HTML>
<html>
...
<h1><?php  echo ($act == 'update') ? 'Update' : 'Create' ?> Elastic Hosts Static VLAN</h1></div><div class="midcont-contbox">
...
<input type="hidden" name="vlan" value="<?php  echo $id ?>" />
</form>
</div>
